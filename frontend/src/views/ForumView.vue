...
<template>
  <main class="flex justify-center min-h-screen pb-12">
    <div class="w-full max-w-5xl mx-auto pt-24 px-4 sm:px-6">
      ...
      <!-- Breadcrumb -->
      <nav class="text-sm text-(--color-text-muted) mb-4 flex items-center gap-1">
        <router-link to="/forums" class="hover:text-sky-600 dark:hover:text-sky-400">Home</router-link>
        <span class="text-(--color-text-muted)">›</span>
        <span class="text-(--color-heading) font-medium" v-if="forumData">{{ forumData.name }}</span>
      </nav>
      ...
      <div v-if="isLoading" class="space-y-3">
        <div v-for="i in 5" :key="i" class="glass p-5 animate-pulse">
          <div class="flex gap-4">
            <div class="w-10 h-10 rounded-full bg-(--color-background-mute) flex-shrink-0"></div>
            <div class="flex-1">
              <div class="h-4 bg-(--color-background-mute) rounded w-2/3 mb-2"></div>
              <div class="h-3 bg-(--color-background-mute) rounded w-1/3"></div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div v-else class="glass overflow-hidden">
        ...
        <!-- Column Headers -->
        <div
          class="hidden md:grid grid-cols-12 gap-2 h-10 bg-(--color-background-soft) border-b border-(--color-border) text-xs uppercase tracking-wider font-semibold text-(--color-text-muted) px-5 items-center select-none">
        ...
        <!-- Empty -->
        <div v-if="threads.length === 0" class="p-10 text-center text-(--color-text-muted)">
          ...
          <p class="font-medium text-(--color-text-muted)">No threads yet</p>
        ...
        <div v-for="thread in threads" :key="thread.id"
          ...
          <div class="col-span-12 md:col-span-5 flex items-start gap-3 min-w-0 pt-5">
            ...
            <div class="flex-shrink-0 mt-0.5 relative">
              <div :class="[
                ...
                isThreadUnread(thread) ? 'bg-sky-500/15 text-sky-600 dark:text-sky-400' : 'bg-(--color-background-mute) text-(--color-text-muted)'
              ]">
            ...
            <div class="min-w-0 flex-1">
              <div class="flex items-center gap-1.5 flex-wrap mb-0.5">
                <span v-if="thread.isPinned"
                  class="inline-flex items-center px-1.5 py-0.5 rounded text-xs font-semibold bg-amber-500/15 text-amber-700 dark:text-amber-300">📌</span>
                <span v-if="thread.isLocked"
                  class="inline-flex items-center px-1.5 py-0.5 rounded text-xs font-semibold bg-red-500/15 text-red-700 dark:text-red-300">🔒</span>
              </div>
              <router-link :to="`/thread/${thread.id}`" :class="[
                'font-bold text-base leading-tight block',
                isThreadUnread(thread)
                  ? 'text-sky-700 dark:text-sky-300 group-hover:text-sky-600 dark:group-hover:text-sky-200'
                  : 'text-(--color-heading) group-hover:text-sky-600 dark:group-hover:text-sky-400'
              ]" class="transition-colors">
                {{ thread.title }}
              </router-link>
              <!-- Mobile info -->
              <div class="flex items-center gap-2 mt-1 md:hidden text-xs text-(--color-text-muted) flex-wrap">
          ...
          <div class="hidden md:flex col-span-2 justify-center items-center">
            <span class="text-sm text-(--color-text-muted) truncate text-center">@{{ thread.author.name }}</span>
          </div>

          <!-- Replies -->
          <div class="hidden md:flex col-span-1 justify-center items-center">
            <span class="text-sm font-semibold text-(--color-text)">{{ thread.replyCount ?? 0 }}</span>
          </div>
          ...
          <div class="hidden md:flex col-span-4 items-center gap-2 min-w-0">
            <div v-if="thread.lastPostAt || thread.lastPostAuthor" class="min-w-0 flex-1">
              <p class="text-xs font-medium text-(--color-text) truncate">
                <span class="text-(--color-text-muted)">by</span> @{{ thread.lastPostAuthor ?? thread.author.name }}
              </p>
              <p :class="['text-xs', isThreadUnread(thread) ? 'text-orange-500 font-semibold' : 'text-(--color-text-muted)']">
                {{ formatTimeAgo(thread.lastPostAt ?? thread.createdAt) }}
              </p>
            </div>
            <div v-else class="text-xs text-(--color-text-muted) italic">No replies yet</div>

            <!-- Mark as Read button — hover reveal -->
            <button v-if="isThreadUnread(thread)" @click="markThreadRead(thread, $event)" title="Mark as read"
              class="flex-shrink-0 w-6 h-6 rounded-full flex items-center justify-center text-(--color-text-muted) hover:text-sky-600 dark:hover:text-sky-300 hover:bg-sky-500/10 transition-all opacity-0 group-hover:opacity-100">
          ...
          <button v-if="isThreadUnread(thread)" @click="markThreadRead(thread, $event)"
            class="absolute top-3 right-3 md:hidden text-xs text-sky-600 dark:text-sky-400 border border-sky-500/30 px-2 py-0.5 rounded-full">
            Mark read
          </button>
        </div>
      </div>
