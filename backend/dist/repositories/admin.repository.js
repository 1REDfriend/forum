...
export class AdminRepository {
    ...
    async getAllUsers(page, limit, search) {
        ...
        const baseQuery = db.select({
            ...
            threadCount: sql `(SELECT COUNT(*) FROM threads WHERE threads.author_id = users.id)::int`,
            postCount: sql `(SELECT COUNT(*) FROM posts WHERE posts.author_id = users.id)::int`,
        }).from(users);
        const filteredQuery = search
            ? baseQuery.where(or(ilike(users.name, `%${search}%`), ilike(users.email, `%${search}%`)))
            : baseQuery;
    ...
    // ─── Forums ─────────────────────────────────────────────────────────────────
    async getAllForumsAdmin(page, limit) {
        const offset = (page - 1) * limit;
        const data = await db.select({
            ...
            creatorName: sql `(SELECT users.name FROM users WHERE users.id = forums.created_by)`,
        })
            .from(forums)
    ...
    // ─── Threads ────────────────────────────────────────────────────────────────
    async getAllThreadsAdmin(page, limit, search) {
        const offset = (page - 1) * limit;
        const baseQuery = db.select({
            id: threads.id,
            title: threads.title,
    ...
    // ─── Posts ──────────────────────────────────────────────────────────────────
    async getAllPostsAdmin(page, limit) {
        const offset = (page - 1) * limit;
        const data = await db.select({
            id: posts.id,
